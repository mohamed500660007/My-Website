...
<div class="p-4 md:p-8 max-w-[1440px] mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">تعديل الخدمة</h1>
            <p class="text-slate-500 text-sm mt-1">تعديل بيانات الخدمة الحالية</p>
        </div>
        ...
        <form id="serviceForm" onsubmit="submitForm(event)" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <input type="text" id="title" name="title" value="{{ $service->title }}" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">{{ $service->description }}</textarea>
                    <p class="text-xs text-red-500 hidden" id="error-description"></p>
                </div>

                <!-- Icon -->
                <div class="space-y-2">
                    <label for="icon" class="block text-sm font-bold text-slate-700">أيقونة الخدمة <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <input type="text" id="icon" name="icon" value="{{ $service->icon }}" required class="w-full px-4 py-2 pr-10 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
                        <span class="material-symbols-outlined absolute right-3 top-2.5 text-slate-400">{{ $service->icon }}</span>
                    </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" id="order_index" name="order_index" value="{{ $service->order_index }}" placeholder="اختياري" class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all">
...
<script>
    async function submitForm(e) {
        ...
        try {
            const response = await fetch('{{ route('admin.services.update', $service->id) }}', {
                method: 'PUT',
                headers: {
